...
client.on('ready', function(){ // Leaked by [ @M3a4x ]
    var ms = 30000 ;
    var setGame = [`$help |  `,` HeroMC By Clyde !`];
    var i = -1;
    var j = 0;
...
message.channel.send(`let args = message.content.split(" ").slice(#{args}).join(" ")`);
    ...
    if(message.content.startsWith('$' + "server")) { // Leaked by [ @M3a4x ]
...
var message = "zg";
         const embed = new Discord.RichEmbed()
     .setColor("GOLD")
     .setTitle('HeroMC Bot Link Click Here .!')
     .setURL('I'm a Praivte bot :sob: :broken_heart:')
  .setDescription(`**
  New Server Add HeroMC Bot ✅
...
var message = "zg";
     ...
     .setURL('I'm a Praivte bot :sob: :broken_heart:')
  .setDescription(`**
  Server Kicked HeroMC Bot :cry:
اسم السيرفر: #{guild.name}
صاحب السيرفر: #{guild.owner}**`);
client.channels.get("430678379495292949").sendEmbed(embed)
});
...
if (message.content.startsWith('-bcall')){
 if (message.author.id !== '300300385988509698') return message.reply('** هذا الأمر قفط لصاحب البوت و شكراًً **')
 if(!message.author.id === '353574347216257025') return;
...
client.on('message', message => { // Leaked by [ @M3a4x ]
if(message.content === '$' + 'مصحف' || message.content === prefix + 'ms7f') {
